...
'
echo " " 
echo -e "$red          ▶Coded by$grn Tanmay Tiwari / Silent Programmer$red◀$rset"
echo -e "$red                      ⫸$ylo  Silent Programmer$red ⫷$rset"
echo 
# Functions
...
then
am start -a android.intent.action.VIEW -d https://www.youtube.com/channel/UC30jlj5G8uXu-sBugpDRfbQ > /dev/null 2>&1
banner
menu
elif [ $optnz = "6" ];
then
am start -a android.intent.action.VIEW -d https://remote-coders-2022.netlify.app/ > /dev/null 2>&1
banner
menu
elif [ $optnz = "7" ];
then
am start -a android.intent.action.VIEW -d https://api.whatsapp.com/send/?phone=+916306256015&text=*Hii* > /dev/null 2>&1
banner
menu
elif [ $optnz = "8" ];
then
...
echo -e '\e[91m
         ...
         ────▌──▐────
'
echo -e "\e[93m                  CALL ME \e[92mSILENT PROGRAMMER" | pv -qL 10
echo -e "\e[93m                  IAM FROM \e[92mREMOTE CODERS" | pv -qL 10
...
echo -e "\e[93m        MY WEBSITE:\e[92m remote-coders-2022.netlify.app\e[0m" | pv -qL 10
...
while git clone https://github.com/Tanmay-Tiwaricyber/t-infect 2> /dev/null; do 
    printf  "\e[92m▓▓▓▓▓▓▓▓▓▓▓▓▓\e[0m"
    sleep 1
done
printf "\e[96m]\e[0m"
...
if [ "$p" = "y" ];
then
am start -a android.intent.action.VIEW -d https://remote-coders-2022.netlify.app/ 2>/dev/null
clear
baner1
menu
elif [ "$p" = "t" ];
